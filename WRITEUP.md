...
**What it is:** A real-time payments fraud detection pipeline built from scratch as a data engineering portfolio project. Simulated card transactions flow through Kafka into a PySpark Structured Streaming medallion architecture (bronze → silver → gold Delta tables), with fraud signals computed per card, high-confidence alerts published to a Kafka topic for downstream consumers, and monitoring via Prometheus + Grafana. The full pipeline also runs on Databricks Free Edition.
...
PySpark Structured Streaming
    ...
    └── Gold    →  per-card fraud signals + fraud_score
              ...
              (score ≥ 0.70, downstream consumers subscribe)
...
The gold layer needs to: (1) compute per-card statistics, (2) join those statistics back to individual transactions, and (3) publish high-confidence alerts to a Kafka topic from the driver. None of this fits the native `groupBy().agg()` streaming model cleanly.

`foreachBatch` gives you the full batch DataFrame API inside each micro-batch. The tradeoff: you lose Spark's built-in state management for windowed aggregations across batches. That's acceptable here because fraud scoring is done within each micro-batch (we care about velocity within the *current* batch window, not across all time), and the Kafka publish runs on the driver where the confluent-kafka Producer lives; it is not serializable to executors.
...
## 6. Databricks Portability Decisions

### 6.1 `DELTA_BASE_PATH` as the single portability lever
...
### 6.2 `trigger(availableNow=True)` on Databricks
...
### 6.3 Unity Catalog auto-detection
...
### 7.2 Silver `DELTA_SCHEMA_NOT_SET` on startup
...
### 7.3 Gold `JAVA_GATEWAY_EXITED` (out of memory)
...
### 7.4 Grafana "No data" on all panels
...
## 9. What I Would Do Differently at Production Scale
...
5. **Alert deduplication.** The current gold job publishes one alert per high-scoring transaction. At high fraud rates a single card could generate dozens of alerts per minute. A TTL-based dedup layer (tracking recently-alerted card IDs) would suppress redundant alerts.
